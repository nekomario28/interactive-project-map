...
Status: **production semantics proven; Galaxy presentation restored to one-world coherence on 2026-08-25 JST**
...
The visual metaphor is **one galaxy with an external halo**, not a graph plus a detached external-repository panel. Contributed is semantically external but visually inhabits the same world-space scene, center, and motion language as the owned project galaxy.
...
Galaxy Systems uses a deliberately slower halo period than its local repository motion, so Contributed reads as distant external context rather than as another owned subsystem. Classic and Hybrid keep the same semantic geometry with style-appropriate periods. Reduced-motion preferences pause this extra motion.
