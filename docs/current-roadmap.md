...
- a single **low-frequency procedural disc-haze texture** adds broad surface depth beneath the semantic disc without adding star points. It is generated once as a 128×128 CanvasTexture, rendered on one 312-unit transparent/additive disc mesh, is explicitly non-semantic and non-pickable, and keeps the nucleus clear: texture contribution is absent through normalized `r = 0.12` (~37 renderer units), fades in by `r = 0.22` (~69 units), then fades toward the outer edge. PR #345 makes only this haze alpha softly arm-aware by reusing the existing `galaxyArmCount(graph)` and 22° `GALAXY_LOG_PITCH`; no second arm model is introduced. The bounded modulation `0.79 + 0.56×armWave²` has angular mean 1.0, redistributing existing haze from inter-arm regions toward arm regions without increasing average haze opacity. Texture/world-Z handedness is explicitly accounted for after the disc rotation (`atan2(-nz, nx)`). PR #353 keeps that arm-aware haze phase-locked to the same **2400 s** spiral-dust pattern by applying the exact same pattern delta to the haze's local-Z rotation, which is the flattened CircleGeometry's world-Y pattern axis. PR #357 also evaluates the haze logarithmic spiral against the scaled dust's world-space reference radius, `42 × 1.08 = 45.36` renderer units, removing the previous constant ~10.9° phase offset without changing dust vertices, haze geometry, opacity, pitch or arm count. Motion Off freezes both together. This remains renderer-local readability/presentation, not a physical stellar/gas/dust density map;
...
- Motion Off freezes semantic node orbits and Galaxy arm-pattern presentation (spiral dust plus phase-locked haze); the existing reduced-motion-derived default remains respected;
- Cosmic / Aurora / Wireframe retain their existing non-Galaxy layout/motion/edge policy.

The rich Chromium Galaxy evidence uses multi-category + Contributed fixtures. Runtime evidence proves co-rotation, outer slowdown, external-lane separation, initial logarithmic pitch, trailing orientation, actual category/repository/Contributed movement, the 0.68 local ellipse model and 2D-Hybrid-aligned local period/direction grammar, Motion Off freeze, inertial starfield behavior, `edgePolicy: "no-persistent-lines"` with `persistentEdgeObjects: 0`, separate arm-pattern motion, the current visual corotation boundary, the qualified central morphology, and the procedural disc-texture marker. Rendered before/after evidence covers all current clutter/depth changes: removing persistent membership chords reduces trajectory-like clutter, PR #340's sparse glow-dominant bulge plus nucleus-only dust fade makes the owner/centre visibly cleaner, PR #342 adds a subtle broad surface contribution while keeping the nucleus clear, and PR #345's final normal render remains free of obvious texture bands while enhanced pixel-difference evidence shows the haze contribution follows the existing logarithmic-arm winding. PR #353 Chromium motion evidence proves the dust and haze pattern rotations remain phase-locked while motion advances and both remain fixed under Motion Off. PR #357 Chromium evidence additionally proves `dustPatternReferenceRadius` and `hazePatternReferenceRadius` are both **45.36**, so the arm-aware haze and scaled dust share the same initial world-space logarithmic phase; its rich render remains visually stable with only the intended subtle haze redistribution. Semantic nodes and spiral dust remain visually dominant.
