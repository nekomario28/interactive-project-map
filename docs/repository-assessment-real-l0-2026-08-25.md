...
Status: **observed portfolio snapshot / evidence vectors only / no composite Quality, Impact, Scale, Personal Contribution, or Prominence score claimed**
...
L0 therefore has strong semantic-routing and basic GitHub-native Impact coverage while intentionally retaining weak authorship/lifecycle coverage.
...
## Zero-extra-API evidence available at L0

Every repository node in this snapshot already contains observed `stars` and `forks`. The adapter can therefore emit a partial Impact evidence vector for all 15 repositories without another GitHub request.

The vector preserves:
...
forks_count
  ...
  -> log1p transformed
  -> not claimed as literal adopter count
```

For fork lineage, absent parent metadata remains an explicit unknown `upstreamContext`; no upstream reputation is invented or copied into local Impact.

The explicit Contributed repository `c0c25034/ProjExD_4` also already carries:
...
Only the evidence channels understood by the person-side extractor are projected at L0: commits and merged pull requests. They become a **partial Personal Contribution evidence vector**, not a Personal Contribution score. Maintainer role, release responsibility, maintained component, review activity, duration, and contribution scope remain unknown.

No project or personal prominence is produced from these partial vectors.
...
The ten forks require local-delta/upstream separation before personal attribution. The one Contributed repository now has direct activity evidence, but activity count alone does not establish contribution magnitude or responsibility.

Quality, Scale, lifecycle assessment, project prominence, personal prominence, and all production scores remain uncollected/null. Impact and Personal Contribution remain evidence vectors with `compositeImpact = null` and `compositePersonalContribution = null`.
...
| `c0c25034/ProjExD_4` | contributed / original / category+artifact unknown | explicit non-ownership; direct activity exists but semantic context/responsibility remain unknown |
...
7. richer Impact inputs only where available and semantically trustworthy; existing stars/forks remain project-side and separate from Quality;
