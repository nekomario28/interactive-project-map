...
The retired `scripts/apply-threejs-galaxy-motion.mjs` compatibility adapter has been removed after its standalone build invocation was retired and repository search found no production caller. Motion constants and the motion kernel must remain in the canonical composer.
...
The two execution-boundary retirements reduce the active post-build mutation chain from 18 to 16 stages. Deleting the now-inactive adapter source files does not change that active count or generated runtime bytes.
