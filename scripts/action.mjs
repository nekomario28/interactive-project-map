...
const STYLE_VALUES = new Set(["radial", "galaxy", "obsidian", "tree", "treemap", "timeline"]);
...
export function safeOutputDir(value = "project-map") { const normalized = String(value || "project-map").trim().replaceAll("\\", "/").replace(/^\.\//, ""); const segments = normalized.split("/").filter(Boolean); if (!segments.length || isAbsolute(normalized) || /^[A-Za-z]:\//.test(normalized) || segments.some((segment) => segment === ".." || segment === ".")) throw new Error("output_dir must be a relative directory without '.' or '..' segments"); return segments.join("/"); }
...
export async function generateStaticMap(config, options = {}) {
  ...
  const outputRoot = resolve(cwd, config.outputDir); const cwdRoot = resolve(cwd) + sep; if (!(outputRoot + sep).startsWith(cwdRoot)) throw new Error("output_dir escaped the workspace");
  await mkdir(outputRoot, { recursive: true });
  const graphPath = posix.join(config.outputDir, "graph.json"); const svgPath = posix.join(config.outputDir, "galaxy.svg"); const absoluteGraphPath = resolve(cwd, graphPath);
  await preserveGeneratedAtWhenUnchanged(absoluteGraphPath, graph); await writeFile(absoluteGraphPath, JSON.stringify(graph, null, 2) + "\n");
  const svg = config.style === "tree" ? renderTreeSvg(graph, config.theme, config.width, config.height)
    : config.style === "radial" ? renderRadialTreeSvg(graph, config.theme, config.width, config.height)
      : config.style === "treemap" ? renderTreemapSvg(graph, config.theme, config.width, config.height)
        : config.style === "timeline" ? renderTimelineSvg(graph, config.theme, config.width, config.height)
          : renderGalaxySvg(graph, config.theme, config.width, config.height, config.style);
  await writeFile(resolve(cwd, svgPath), svg); return { graphPath, svgPath, graph };
}
async function setOutput(name, value) { if (process.env.GITHUB_OUTPUT) await appendFile(process.env.GITHUB_OUTPUT, `${name}=${value}\n`); }
async function main() { if (!process.env.INPUT_GITHUB_TOKEN && !process.env.GITHUB_TOKEN) throw new Error("github_token input is required"); const config = actionConfigFromEnv(); const result = await generateStaticMap(config); await setOutput("svg-path", result.svgPath); await setOutput("graph-path", result.graphPath); console.log(`Generated ${result.graph.repositoryCount} repositories for ${config.username} (${config.style})`); console.log(`SVG: ${result.svgPath}`); console.log(`Graph: ${result.graphPath}`); }
