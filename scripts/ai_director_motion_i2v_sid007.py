...
ROOT=Path('.tmp/ai-director-motion-i2v-sid007'); ROOT.mkdir(parents=True,exist_ok=True)
raw=Path(hf_hub_download(repo_id='UBC-ViL/Spotlight-VideoGen-Errors',repo_type='dataset',filename='test/spotlight/ltx2/sid_007.mp4',local_dir=str(ROOT/'raw')))
anchor=ROOT/'sid_007_anchor_0.45s.jpg'
subprocess.run(['ffmpeg','-y','-v','error','-ss','0.45','-i',str(raw),'-frames:v','1','-q:v','2',str(anchor)],check=True)
prompt=('The same robot on the same table lifts one foot and dances by moving both arms strictly UP AND DOWN VERTICALLY. '
        'Do not swing the arms side to side. Keep the robot identity, table, room, camera, and body stable. Clear vertical arm motion only.')
providers=['Upsampler/ltx-video','Lightricks/ltx-2-distilled']
result={'pid':'sid_007','action':'MOTION_SPECIFIC_ANCHOR_I2V','anchor_sec':0.45,'prompt':prompt,'attempts':[]}
for provider in providers:
 a={'provider':provider}; t=time.time()
 try:
  c=Client(provider,verbose=False)
  out=c.predict(handle_file(str(anchor)),prompt,1.0,False,7007,False,512,768,api_name='/generate_video')
  ...
  for cp in candidates:
   ...
   if p.exists() and p.suffix.lower() in {'.mp4','.mov','.webm'}:
    copied=ROOT/'sid_007_motion_i2v.mp4'; shutil.copy2(p,copied); break
  if copied:
