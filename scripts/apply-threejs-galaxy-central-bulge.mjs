...
const SCENE_WITH_MORPHOLOGY = `${SCENE_ANCHOR}if(threeStyle==="galaxy")softenGalaxyCentralDust(dust);const galaxyDiscHaze=threeStyle==="galaxy"?createGalaxyDiscHaze(THREE,username):null;if(galaxyDiscHaze){scene.add(galaxyDiscHaze);document.body.dataset.galaxyDiscTexture="procedural-haze-v1";}const galaxyBulge=threeStyle==="galaxy"?createGalaxyCentralBulge(THREE,username,glowTexture):null;if(galaxyBulge){scene.add(galaxyBulge);document.body.dataset.galaxyCentralStructure="bulge";}`;
...
export function patchThreejsGalaxyCentralBulgeRuntime(source) {
  let next = source;
  if (!next.includes("function createGalaxyDiscHaze(THREE,seed)")) {
  ...
  if (next.includes(PREVIOUS_SCENE_WITH_BULGE)) next = next.replace(PREVIOUS_SCENE_WITH_BULGE, SCENE_WITH_MORPHOLOGY);
