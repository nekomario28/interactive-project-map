...
const GALAXY_BULGE_HELPER = `function createGalaxyCentralBulge(THREE,seed,glowTexture){const count=innerWidth<720?180:360,positions=new Float32Array(count*3);for(let index=0;index<count;index+=1){const theta=TAU*hashUnit(seed+":bulge:theta:"+index),vertical=hashUnit(seed+":bulge:vertical:"+index)*2-1,planar=Math.sqrt(Math.max(0,1-vertical*vertical)),radius=44*Math.pow(hashUnit(seed+":bulge:radius:"+index),1.9);positions[index*3]=Math.cos(theta)*planar*radius;positions[index*3+1]=vertical*radius*.72;positions[index*3+2]=Math.sin(theta)*planar*radius;}const geometry=new THREE.BufferGeometry();geometry.setAttribute("position",new THREE.BufferAttribute(positions,3));const material=new THREE.PointsMaterial({color:0xffd8aa,size:innerWidth<720?1.25:1.55,transparent:true,opacity:.24,depthWrite:false,sizeAttenuation:true,blending:THREE.AdditiveBlending}),points=new THREE.Points(geometry,material);points.name="galaxy-bulge-stars";const glowMaterial=new THREE.SpriteMaterial({map:glowTexture,color:0xffcf91,transparent:true,opacity:.12,depthWrite:false,blending:THREE.AdditiveBlending}),glow=new THREE.Sprite(glowMaterial);glow.name="galaxy-bulge-glow";glow.scale.set(88,72,1);glow.position.set(0,1,0);const group=new THREE.Group();group.name="galaxy-central-bulge";group.userData.decorative=true;group.userData.semantic=false;group.add(points,glow);return group;}`;
