...
const EDGE_SYNC = "edgeLines.renderOrder=-1;root.add(edgeLines);}function syncDynamicEdges(){if(!edgeLines)return;const attribute=edgeLines.geometry.getAttribute(\"position\");if(!attribute)return;let cursor=0;for(const edge of graph.edges){const source=nodeMeshes.get(edge.source),targetMesh=nodeMeshes.get(edge.target);if(!source||!targetMesh||!source.visible||!targetMesh.visible)continue;attribute.setXYZ(cursor++,source.position.x,source.position.y,source.position.z);attribute.setXYZ(cursor++,targetMesh.position.x,targetMesh.position.y,targetMesh.position.z);}attribute.needsUpdate=true;}";
const ANIMATE_DUST = "dust.rotation.y+=delta*.0035;";
const ANIMATE_DUST_GALAXY = "dust.rotation.y+=delta*(threeStyle===\"galaxy\"?.0021:.0035);";
