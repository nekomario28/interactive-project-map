...
const GALAXY_EDGE_LOOP_START = "for(const edge of graph.edges){if(threeStyle===\"galaxy\"&&edge.type!==\"membership\")continue;const source=nodeMeshes.get(edge.source),targetMesh=nodeMeshes.get(edge.target);";
const EDGE_END = "edgeLines.renderOrder=-1;root.add(edgeLines);}";
const EDGE_SYNC = "edgeLines.renderOrder=-1;root.add(edgeLines);}function syncDynamicEdges(){if(!edgeLines)return;const attribute=edgeLines.geometry.getAttribute(\"position\");if(!attribute)return;let cursor=0;for(const edge of graph.edges){if(threeStyle===\"galaxy\"&&edge.type!==\"membership\")continue;const source=nodeMeshes.get(edge.source),targetMesh=nodeMeshes.get(edge.target);if(!source||!targetMesh||!source.visible||!targetMesh.visible)continue;attribute.setXYZ(cursor++,source.position.x,source.position.y,source.position.z);attribute.setXYZ(cursor++,targetMesh.position.x,targetMesh.position.y,targetMesh.position.z);}attribute.needsUpdate=true;}";
...
export function patchThreejsGalaxyMotionRuntime(source) {
  ...
  next = replaceRequired(next, EDGE_LOOP_START, GALAXY_EDGE_LOOP_START, "Galaxy persistent membership-edge policy");
