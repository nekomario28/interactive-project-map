...
export async function applyThreejsSearchContext({ siteDir = join(process.cwd(), "site") } = {}) {
  const runtimePath = join(siteDir, "threejs-viewer.js");
  const runtime = await readFile(runtimePath, "utf8");
  const nextRuntime = patchThreejsSearchContextRuntime(runtime);
  if (nextRuntime !== runtime) await writeFile(runtimePath, nextRuntime);
  return {
    runtimePath,
    injected: nextRuntime !== runtime,
  };
}

async function main() {
  const result = await applyThreejsSearchContext();
  console.log(`Applied shared search context adapter to ${result.runtimePath}`);
}
