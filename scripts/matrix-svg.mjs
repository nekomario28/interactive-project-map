...
function aggregate(graph) {
  ...
  let bundles = groups.map((group) => {
    ...
    return { group, members: ownedRepos.filter((repo) => repo.groupId === key || group.id === `group:${repo.groupId}`) };
  }).filter((bundle) => bundle.members.length);
  bundles = addExternalBundle(bundles, repos);
  const rows = bundles.map(({ group, members }) => {
    const cells = columns.map((language) => {
      ...
      for (const repo of cellRepos) counts[repositoryStatus(repo)] += 1;
...
export function renderMatrixSvg(graph, theme, width, height) {
  ...
  rows.forEach((row, rowIndex) => {
    ...
    pieces.push(`<text x="${left - 8}" y="${(y + cellHeight / 2 + 3).toFixed(1)}" text-anchor="end" fill="${row.group.external ? colors.contributed : colors.muted}" font-size="9.5" font-weight="600">${esc(rowLabel)}</text>`);
    row.cells.forEach((cell, columnIndex) => {
      const x = left + cellWidth * columnIndex;
      const intensity = cell.total === 0 ? 0.025 : 0.14 + 0.68 * Math.sqrt(cell.total / max);
      const title = `${row.group.label} × ${cell.language}: ${cell.total} repositories · ${cell.counts.original} original · ${cell.counts.fork} fork · ${cell.counts.archived} archived · ${cell.counts.contributed} contributed`;
      pieces.push(`<g><title>${esc(title)}</title><rect x="${(x + 1.5).toFixed(1)}" y="${(y + 1.5).toFixed(1)}" width="${Math.max(0, cellWidth - 3).toFixed(1)}" height="${Math.max(0, cellHeight - 3).toFixed(1)}" rx="4" fill="${row.group.external ? colors.contributed : colors.heat}" opacity="${intensity.toFixed(2)}" stroke="${colors.grid}" stroke-width="0.8"/>`);
      if (cell.total > 0) {
        const barX = x + 5;
        const barY = y + cellHeight - 7;
        const barWidth = Math.max(4, cellWidth - 10);
        let cursor = barX;
        for (const status of ["original", "fork", "archived", "contributed"]) {
