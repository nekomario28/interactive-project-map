...
window.addEventListener("DOMContentLoaded", () => {
  ...
  const runtime = { initialized: false, nodesRef: null, owner: null, categories: new Map(), repositories: new Map(), globalRotation: 0, lastTime: performance.now(), armCount: 1 };

  const dust = Array.from({ length: 220 }, (_, index) => ({
    arm: hash(`hybrid:dust:arm:${index}`) % 3,
    t: (hash(`hybrid:dust:t:${index}`) % 10000) / 10000,
    offset: ((hash(`hybrid:dust:o:${index}`) % 1000) / 1000 - 0.5) * 0.44,
    radial: ((hash(`hybrid:dust:r:${index}`) % 1000) / 1000 - 0.5) * 36,
    size: 0.45 + (hash(`hybrid:dust:s:${index}`) % 90) / 100,
    alpha: 0.045 + (hash(`hybrid:dust:a:${index}`) % 105) / 1000,
  }));
  ...
  function orbitAssignments(group, members) {
    ...
    while (cursor < members.length) {
      ...
      for (let index = 0; index < take; index += 1) {
        assignments.push({ node: members[cursor + index], lane, semiMajor, semiMinor, phase: seedPhase + TAU * index / Math.max(1, take) + lane * 0.27, period: 480 + lane * 240, direction });
      }
      cursor += take;
      lane += 1;
    }
  ...
  function initialize() {
    ...
    const groups = state.nodes.filter((node) => node.type === "group").sort((a, b) => (memberships.get(b.id)?.length || 0) - (memberships.get(a.id)?.length || 0) || String(a.id).localeCompare(String(b.id)));
    ...
    owner.x = 0; owner.y = 0; owner.vx = 0; owner.vy = 0;
    ...
    const tierGap = Math.max(145, maximumSystemRadius * 1.15 + 80);
    const baseRadius = Math.max(165, maximumSystemRadius + 105);
    ...
    prepared.forEach((preparedCategory, index) => {
      ...
      const category = { node: preparedCategory.group, members: preparedCategory.members, assignments: preparedCategory.assignments, systemRadius: preparedCategory.systemRadius, armIndex, tier, baseAngle, angle: baseAngle, globalRadius: baseRadius + tier * tierGap };
      runtime.categories.set(category.node.id, category);
      for (const target of category.assignments) { target.category = category; runtime.repositories.set(target.node.id, target); }
    ...
    if (!state.selected && detailsDescription) detailsDescription.textContent = motionMedia.matches
  ...
  function step(now) {
    if (state.nodes !== runtime.nodesRef || !runtime.initialized) { if (!initialize()) return false; }
  ...
  function spiralRange() {
    const categories = [...runtime.categories.values()];
    const maxRadius = Math.max(...categories.map((category) => category.globalRadius), 245);
    const minRadius = Math.max(72, Math.min(...categories.map((category) => category.globalRadius), 165) * 0.45);
    return { minRadius, maxRadius };
  }

  function drawSpiralGuides(colors) {
    if (!runtime.categories.size) return;
    const { minRadius, maxRadius } = spiralRange();
    for (let arm = 0; arm < runtime.armCount; arm += 1) {
      const points = [];
      for (let index = 0; index <= 44; index += 1) {
        const t = index / 44;
        const radius = minRadius + t * (maxRadius + 105 - minRadius);
        const angle = -Math.PI / 2 + arm * TAU / runtime.armCount + t * 1.72 + runtime.globalRotation;
        points.push(worldToScreen(Math.cos(angle) * radius, Math.sin(angle) * radius));
      }
      ctx.save();
      ctx.lineCap = "round";
      ctx.lineJoin = "round";
      ...
      ctx.strokeStyle = rgba(colors.group, 0.028);
      ctx.lineWidth = 20;
      ctx.stroke();
      ctx.strokeStyle = rgba(colors.group, 0.065);
      ctx.lineWidth = 2.2;
      ctx.stroke();
      ctx.restore();
    }
  ...
  function drawSpiralDust(colors, width, height) {
    ...
    for (const particle of dust) {
      ...
      const radius = minRadius + particle.t * (maxRadius + 115 - minRadius) + particle.radial;
      ...
      ctx.beginPath(); ctx.arc(point.x, point.y, particle.size, 0, TAU); ctx.fill();
    }
    ctx.globalAlpha = 1;
  }
  ...
  function drawSystems(colors) {
    for (const category of runtime.categories.values()) {
      ...
      ctx.fillStyle = gradient;
      ctx.beginPath(); ctx.arc(center.x, center.y, outer, 0, TAU); ctx.fill();

      const orientation = category.angle + Math.PI / 2;
      ctx.save(); ctx.translate(center.x, center.y); ctx.rotate(orientation);
      ctx.strokeStyle = rgba(colors.group, focus ? 0.31 : 0.13); ctx.lineWidth = focus ? 0.9 : 0.6;
      const lanes = new Map();
      for (const target of category.assignments) if (!lanes.has(target.lane)) lanes.set(target.lane, target);
      for (const target of lanes.values()) { ctx.beginPath(); ctx.ellipse(0, 0, target.semiMajor * state.zoom, target.semiMinor * state.zoom, 0, 0, TAU); ctx.stroke(); }
      ctx.restore();
    }
  }
  ...
  drawBackground = function hybridBackground(colors, width, height) {
    ...
    drawNucleus(colors);
    drawSpiralGuides(colors);
    drawSpiralDust(colors, width, height);
    drawSystems(colors);
  };

  function frame(now) { const changed = step(now); if (changed) draw(); requestAnimationFrame(frame); }
