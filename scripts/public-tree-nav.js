...
(() => {
  ...
  function styleUrl(style) { const normalized = normalize(style); const route = dedicated.has(normalized) ? `../${normalized}/` : "../u/"; const url = new URL(route, location.href); const username = query.get("username"); if (username) url.searchParams.set("username", username); url.searchParams.set("style", normalized); return url; }
