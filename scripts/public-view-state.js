...
(() => {
  ...
  // Existing Galaxy/Obsidian runtimes already honor prefers-reduced-motion.
  // Extend that one source of truth instead of adding a second animation engine.
  if (nativeMatchMedia) {
  ...
  function init() {
    ...
    function parseStatuses(value) {
      ...
      const parsed = String(value).split(",").map((item) => aliases[item] || item).filter((item) => STATUS_VALUES.includes(item));
    ...
    function focusedGraph(graph) {
      ...
      for (const repo of repositories) {
        if (!repositoryIds.has(repo.id)) continue;
        if (repo.groupId) groupIds.add(String(repo.groupId).startsWith("group:") ? String(repo.groupId) : `group:${repo.groupId}`);
      }
      for (const edge of Array.isArray(graph.edges) ? graph.edges : []) {
      ...
      return {
        ...
        nodes,
        edges,
        ...(semanticEdges.length ? { semanticEdges } : { semanticEdges: [] }),
    ...
    function visibleRepositories() {
      return state.nodes.filter((node) => node?.type === "repository" && statusVisible(node));
    }
    ...
    function updateControls() {
      ...
      motionButton.classList.toggle("is-active", !effectiveMotionOff);
      ...
      motionButton.title = nativeReducedMotion() ? "Motion is disabled by the operating-system reduced-motion preference." : "Toggle graph motion.";
      activityButton.classList.toggle("is-active", activity);
      ...
      for (const button of focusDepthButtons) {
        const active = Number(button.dataset.focusDepth) === focusDepth;
        button.classList.toggle("is-active", active);
        button.setAttribute("aria-pressed", String(active));
      }
      if (resultCount) {
        const visible = visibleRepositories().length;
        const total = state.nodes.filter((node) => node?.type === "repository").length;
        resultCount.textContent = `${visible} / ${total} repos`;
      }
    }
    ...
    shareButton.addEventListener("click", async () => {
      ...
      try {
        ...
        // The URL already reflects the current view; no fallback mutation is needed.
      }
    });
