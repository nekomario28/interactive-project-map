...
function normalizedList(value, maxItems, maxChars) {
...
export function buildRepoSemanticDocument(repo) {
  return {
...
export function buildRepoSemanticDocuments(repos) {
  return Array.isArray(repos) ? repos.map(buildRepoSemanticDocument) : [];
}

export function canonicalSemanticDocument(document) {
...
export function semanticDocumentText(document) {
  const lines = [
    `name: ${document.name}`,
