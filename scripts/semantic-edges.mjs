import { DISABLED_EMBEDDING_PROVIDER, embedSemanticDocuments } from "./embedding.mjs";
...
function boundedMaxEdges(value, documents, topK) {
...
function cosineSimilarity(a, b) {
  if (!Array.isArray(a) || !Array.isArray(b) || a.length !== b.length || a.length === 0) throw new Error("Semantic vectors must have one consistent non-zero dimension");
  let dot = 0;
  let normA = 0;
  let normB = 0;
...
function candidateSort(documents) {
  return (a, b) => b.score - a.score || documents[a.index].name.localeCompare(documents[b.index].name);
}

function retainCandidate(target, candidate, topK, documents) {
...
export function buildSparseSemanticEdges(documents, vectors, options = {}) {
  ...
  const neighbors = Array.from({ length: documents.length }, () => []);
  let comparisons = 0;
...
export async function generateSemanticEdges(repos, provider = DISABLED_EMBEDDING_PROVIDER, cache, options = {}) {
  ...
  try {
    ...
    if (embedded.diagnostics.disabled || documents.length < 2) {
      return {
        ...
        diagnostics: { documents: documents.length, comparisons: 0, retainedCandidates: 0, emittedEdges: 0, topK, minSimilarity, maxEdges },
      };
    }
