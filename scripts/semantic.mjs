...
export const FRAMEWORK_HINTS = Object.freeze([
...
const MANIFEST_CATEGORY_HINTS = new Map();
...
export function semanticKeywordMatches(text, keyword) {
...
function canonicalFrameworkId(value) {
...
export function extractFrameworkIdentifiers(raw) {
...
function addEvidence(target, seen, categoryId, source, value, path) {
...
function matchRuleAliases(value, source, target, seen) {
  if (!value) return;
  for (const rule of GROUP_RULES) {
...
function matchTopics(topics, target, seen) {
...
function matchManifests(manifests, target, seen) {
...
function matchFrameworks(frameworks, target, seen) {
...
function classificationConfidence(topScore, secondScore, winningEvidence) {
...
function evidenceSort(a, b) {
  return b.weight - a.weight || a.categoryId.localeCompare(b.categoryId) || a.source.localeCompare(b.source) || a.value.localeCompare(b.value);
}

export function classifyRepository(repo) {
  if (repo?.classification && typeof repo.classification === "object") return repo.classification;
...
export function classificationRuleIds() {
