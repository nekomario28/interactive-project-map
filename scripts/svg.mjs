...
export function renderGalaxySvg(graph, theme, width, height) {
  ...
  const nodes = points.map(({ x, y, node }) => {
    ...
    return `<g><circle cx="${x}" cy="${y}" r="${radius.toFixed(1)}" fill="${fill}" opacity="${node.fork ? 0.72 : 0.95}"/><text x="${x}" y="${y + radius + 12}" text-anchor="middle" fill="${fg}" font-size="10">${esc(label)}</text></g>`;
  }).join("");
