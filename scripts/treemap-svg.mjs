...
export function renderTreemapSvg(graph, theme, width, height) {
  ...
  for (const bundle of groupBoxes) {
    ...
    for (const item of repoBoxes) {
      ...
      pieces.push(`<g><title>${esc(repo.label)} · ${status}</title><rect x="${(r.x + 1).toFixed(1)}" y="${(r.y + 1).toFixed(1)}" width="${Math.max(0, r.width - 2).toFixed(1)}" height="${Math.max(0, r.height - 2).toFixed(1)}" rx="4" fill="${fill}" opacity="${repo.archived ? "0.68" : repo.fork ? "0.78" : "0.90"}"${repo.archived ? ` stroke="${fill}" stroke-width="1.2" stroke-dasharray="3 2"` : ""}/>`);
