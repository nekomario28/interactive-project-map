...
async function validateDynamicMarkup() {
  ...
  const treemapViewer = await readFile(join(siteDir, "treemap", "index.html"), "utf8");
  ...
  if (!/<a id="openMap" class="button" href="\.\/radial\/" target="_blank" rel="noopener">/.test(home)) throw new Error("Generator interactive-map anchor must have the classic radial safe default href and rel=noopener");

  const sourceLessImages = [...home.matchAll(/<img\b[^>]*>/gi)].filter((match) => !/\bsrc\s*=/.test(match[0]));
  if (sourceLessImages.length !== 1 || !/\bid="preview"/.test(sourceLessImages[0][0]) || !/\balt=/.test(sourceLessImages[0][0])) throw new Error("Only the JS-populated preview image may omit src, and it must retain alt text");

  for (const [name, html] of [["viewer", viewer], ["radial viewer", radialViewer], ["tree viewer", treeViewer], ["treemap viewer", treemapViewer]]) {
    ...
    if (!/<link rel="stylesheet" href="\.\.\/viewer\.css">/.test(html)) throw new Error(`${name} must load the external validated stylesheet`);
  ...
  if (!/tree-router\.js/.test(viewer) || !/viewer\.js/.test(viewer)) throw new Error("Shared viewer must route dedicated styles before starting the graph viewer");
  if (!/radial-viewer\.js/.test(radialViewer)) throw new Error("Radial viewer must load its classic browser renderer");
  if (!/tree-nav\.js/.test(treeViewer) || !/tree-viewer\.js/.test(treeViewer)) throw new Error("Tree viewer must load hierarchical rendering plus dedicated-view navigation");
  if (!/treemap-viewer\.js/.test(treemapViewer)) throw new Error("Treemap viewer must load its rectangle-layout browser renderer");
...
async function generateWorkflowFixture() {
  ...
  const workflow = vm.runInContext("workflowFor({username:'nekomario28',theme:'dark',style:'treemap',maxRepos:100,forks:true,archived:false})", context);
  ...
  if (!workflow.includes("style: treemap")) throw new Error("Generated installer workflow must preserve Treemap style");
  if (workflow.includes("__PROJECT_MAP_ACTION_REF__")) throw new Error("Generated installer workflow still contains the Action ref placeholder");
...
console.log("Generated Pages markup, CSS, five style presets and browser installer runtime validated.");
