...
export function renderGalaxySvg(graph: GalaxyGraph, theme: Theme, width: number, height: number): string {
  ...
  const nodes = points.map(({ x, y, node }) => {
    ...
    return `<g><circle cx="${x}" cy="${y}" r="${r.toFixed(1)}" fill="${fill}" opacity="${node.fork ? 0.72 : 0.95}"/><text x="${x}" y="${y + r + 12}" text-anchor="middle" fill="${fg}" font-size="10">${esc(label)}</text></g>`;
  }).join("");
