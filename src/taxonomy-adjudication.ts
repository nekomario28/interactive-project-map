...
import type {
  ...
  TaxonomyAdjudicationCase,
  TaxonomyAdjudicationDiagnostics,
  TaxonomyAdjudicationProvider,
  TaxonomyAdjudicationResult,
...
export const DISABLED_TAXONOMY_ADJUDICATOR: TaxonomyAdjudicationProvider = Object.freeze({
  id: "disabled",
  model: "disabled",
  async adjudicate(): Promise<unknown> {
    throw new Error("Disabled taxonomy adjudicator must not be invoked");
  },
});
...
function providerIdentity(provider: TaxonomyAdjudicationProvider): { id: string; model: string } {
  const id = String(provider?.id ?? "").trim();
  const model = String(provider?.model ?? "").trim();
  if (!id || id.length > 120) throw new Error("Taxonomy adjudicator id must be 1-120 characters");
  if (!model || model.length > 180) throw new Error("Taxonomy adjudicator model must be 1-180 characters");
  return { id, model };
}
...
function buildCases(
  ...
  const byName = new Map(repos.map((repo) => [repoKey(repo.name), repo]));
  ...
  return selected.map((repo, index) => {
    ...
    return {
      ...
      categories: taxonomy.categories.map((category) => ({
        ...
        aliases: [...category.aliases].slice(0, 32),
...
function validateDecision(
...
): { assignment?: RepositoryTaxonomyAssignment; invalid: boolean; declined: boolean } {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return { invalid: true, declined: false };
  const item = raw as Record<string, unknown>;
  if (String(item.repoName ?? "") !== expectedRepoName) return { invalid: true, declined: false };
  const confidence = item.confidence;
  if (typeof confidence !== "number" || !Number.isFinite(confidence) || confidence < 0 || confidence > 1) {
    return { invalid: true, declined: false };
  }
  const categoryId = item.categoryId == null ? null : String(item.categoryId).normalize("NFKC").trim().toLocaleLowerCase("en-US");
  ...
  const reason = String(item.reason ?? "").normalize("NFKC").replace(/\s+/gu, " ").trim().slice(0, 240);
  if (!reason) return { invalid: true, declined: false };
  const secondaryTags = boundedTags(item.secondaryTags);
  return {
    invalid: false,
    declined: false,
    assignment: {
      ...
      evidence: [{
        ...
        source: "llm",
        value: reason,
        weight: Math.round(confidence * 1_000_000) / 1_000_000,
      }],
    },
  };
}

export async function adjudicateAmbiguousTaxonomyAssignments(
  repos: GitHubRepo[],
  taxonomy: PortfolioTaxonomy | null | undefined,
  base: TaxonomyAssignmentResult,
  provider: TaxonomyAdjudicationProvider | null | undefined = DISABLED_TAXONOMY_ADJUDICATOR,
  options: {
    minConfidence?: number;
    maxCases?: number;
    batchSize?: number;
  } = {},
): Promise<TaxonomyAdjudicationResult> {
  ...
  try {
    ...
    return {
      ...
      diagnostics: {
        eligible: base.ambiguous.length, attempted: 0, accepted: 0, declined: 0, invalid: 0, remaining: base.ambiguous.length,
        capped: false, calls: 0, providerId: "invalid", model: "invalid", disabled: false,
      },
  ...
  if (!taxonomy || !eligible || identity.id === "disabled") {
    return {
      ...
      diagnostics: {
        eligible, attempted: 0, accepted: 0, declined: 0, invalid: 0, remaining: eligible,
        capped: false, calls: 0, providerId: identity.id, model: identity.model, disabled: identity.id === "disabled",
      },
    };
  }
  ...
  for (let offset = 0; offset < cases.length; offset += batchSize) {
    ...
    try {
      calls += 1;
      raw = await chosen.adjudicate(batch);
    } catch (error) {
      errorMessage = String(error instanceof Error ? error.message : error).slice(0, 300);
      break;
    }
    if (!Array.isArray(raw) || raw.length !== batch.length) {
      errorMessage = `Taxonomy adjudicator returned ${Array.isArray(raw) ? raw.length : "invalid"} decisions for ${batch.length} cases`;
      break;
    }
    for (let index = 0; index < batch.length; index += 1) {
      ...
      if (checked.declined || !checked.assignment) {
        declined += 1;
        continue;
      }
  ...
  const diagnostics: TaxonomyAdjudicationDiagnostics = {
    eligible,
    attempted,
    accepted,
    declined,
  ...
  return {
    assignments,
    ambiguous: [...remaining].sort((a, b) => a.localeCompare(b)),
    diagnostics,
