...
const graph = {
  ...
  nodes: [
    ...
    ...[
      ["robot-one", "robotics", "Python", 8], ["robot-two", "robotics", "C++", 4], ["robot-three", "robotics", "Rust", 2],
      ["ai-one", "ai", "Python", 7], ["ai-two", "ai", "JavaScript", 3], ["ai-three", "ai", "Python", 1],
    ].map(([name, groupId, language, stars]) => ({
      id: `repository:${name}`, label: name, type: "repository", url: `https://github.com/example/${name}`, description: `${name} regression fixture`, language,
...
async function capturedCanvasLabels(page) {
  return page.evaluate(() => {
    const seen = []; const original = ctx.fillText;
    ctx.fillText = function capture(text, ...args) { seen.push(String(text)); return original.call(this, text, ...args); };
...
test("Galaxy Classic keeps normal-size repository labels visible while orbiting globally", async ({ page }) => {
  ...
  const before = await page.evaluate(() => Object.fromEntries(state.nodes.filter((node) => node.type === "repository").map((node) => [node.id, { x: node.x, y: node.y }])));
  await page.waitForTimeout(1300);
  const after = await page.evaluate(() => Object.fromEntries(state.nodes.filter((node) => node.type === "repository").map((node) => [node.id, { x: node.x, y: node.y }])));
  ...
  await page.screenshot({ path: ".tmp/playwright-visual/dark/galaxy-classic-selected.png", fullPage: true });
...
test("Obsidian drag reheats the whole graph with no release anchor", async ({ page }) => {
  ...
  await page.screenshot({ path: ".tmp/playwright-visual/dark/obsidian-global-reheat.png", fullPage: true }); expect(failures).toEqual([]);
});

test("Tree keeps every repository child on exactly one parallel level", async ({ page }) => {
  await installGraph(page); const failures = browserErrors(page);
  await page.goto("/tree/?username=example&style=tree"); await expect(page.locator("#status")).toBeHidden();
  const levels = await page.evaluate(() => [...new Set(state.nodes.filter((node) => node.type === "repository").map((node) => node.y.toFixed(4)))]);
  expect(levels).toHaveLength(1); await page.screenshot({ path: ".tmp/playwright-visual/dark/tree-parallel.png", fullPage: true }); expect(failures).toEqual([]);
});

test("Sunburst prints every repository name for a normal-size portfolio", async ({ page }) => {
  await installGraph(page); const failures = browserErrors(page);
  await page.goto("/sunburst/?username=example&style=sunburst"); await expect(page.locator("#status")).toBeHidden();
  const labels = await capturedCanvasLabels(page); for (const name of repoNames) expect(labels).toContain(name);
  await page.screenshot({ path: ".tmp/playwright-visual/dark/sunburst-labels.png", fullPage: true }); expect(failures).toEqual([]);
});

test("style query text is authoritative and legacy galaxy aliases to Systems", async ({ page }) => {
  await installGraph(page); const failures = browserErrors(page);
  await page.goto("/tree/?username=example&style=galaxy");
  await page.waitForURL(/\/u\/\?username=example&style=galaxy-systems/);
  await expect(page.locator("#style")).toHaveValue("galaxy-systems"); await expect(page.locator("#status")).toBeHidden();
  await page.goto("/u/?username=example&style=tree");
  await page.waitForURL(/\/tree\/\?username=example&style=tree/);
  await expect(page.locator("#style")).toHaveValue("tree"); await expect(page.locator("#status")).toBeHidden(); expect(failures).toEqual([]);
});
