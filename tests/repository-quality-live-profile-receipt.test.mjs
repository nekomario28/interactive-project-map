...
test("bounded Quality enrichment keeps live membership 15 -> 15 while five evidence sources yield four portfolio overlays", () => {
...
test("current-profile Quality overlay summaries preserve repository-snapshot and local-delta scopes", () => {
  const result = runLiveProjection();
  ...
  const antiSecurity = antifullbright.views.detail.segments.find((segment) => segment.id === "security-safety");
  assert.equal(antiSecurity.applicability, "optional");
  assert.equal(antiSecurity.findingState, "supports");
