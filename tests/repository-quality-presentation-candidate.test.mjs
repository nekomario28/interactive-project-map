...
test("CLI refuses to overwrite graph, assessment, or diagnostics paths", () => {
  ...
  try {
    ...
    assert.throws(() => runRepositoryQualityPresentationCandidateCli(["--graph", graphPath, "--assessment", assessmentPath, "--out", assessmentPath]), /must not overwrite assessment input/);
    const outPath = path.join(temp, "presentation.json");
    assert.throws(() => runRepositoryQualityPresentationCandidateCli(["--graph", graphPath, "--assessment", assessmentPath, "--out", outPath, "--diagnostics-out", outPath]), /diagnostics output path must differ/);
  } finally {
    fs.rmSync(temp, { recursive: true, force: true });
  }
});
