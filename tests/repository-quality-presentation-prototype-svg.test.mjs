...
test("compact current-profile Quality prototype renders all fifteen repositories with attribution-safe 4/11 availability", () => {
  ...
  assert.match(svg, /<title>antifullbright<\/title>/);
  assert.match(svg, />5\/6 interpreted<\/text>/);
...
test("detail current-profile Quality prototype preserves dimension identity for four attribution-safe repositories", () => {
