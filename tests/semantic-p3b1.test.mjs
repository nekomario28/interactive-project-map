...
async function taxonomyFor(repos) {
  const resolved = await resolvePortfolioTaxonomy(repos, {
    id: "fixture-taxonomy",
    model: "v1",
    async discover() { return { categories }; },
  });
...
function classification(categoryId, confidence, secondaryTags = []) {
  return {
    categoryId,
    categoryLabel: categoryId === "robotics" ? "Robotics / ROS 2" : "Other",
    secondaryTags,
    confidence,
    method: "deterministic",
    evidence: [{ categoryId, source: "description", value: `${categoryId} evidence`, weight: confidence }],
  };
}
...
function vectorForText(text) {
  ...
  if (/^name: robot$/m.test(text)) return [0.995, 0.05];
  if (/^name: web$/m.test(text)) return [0.05, 0.995];
...
test("taxonomy category canonical text/cache identity is stable and source/script implementations match", async () => {
  ...
  const sourceKey = await sourceCategoryKey(category, provider);
  assert.equal(sourceKey, scriptKey);
  assert.match(scriptKey, /^embedding:taxonomy-category-v1:/);

  const reordered = await scriptCategoryKey({ ...category, aliases: ["ROS2", "Gazebo"] }, provider);
  assert.equal(reordered, scriptKey, "alias order must not change category cache identity");
  const relabeled = await scriptCategoryKey({ ...category, description: `${category.description} sim-to-real` }, provider);
  assert.notEqual(relabeled, scriptKey);
...
test("repository assignment overrides are normalized strictly and source/script implementations match", () => {
  const input = {
    Robot: { categoryId: "ROBOTICS", secondaryTags: ["sim-to-real", "SIM-TO-REAL", "manipulation"] },
  };
  const script = normalizeScriptRepoOverrides(input);
  const source = normalizeSourceRepoOverrides(input);
  assert.deepEqual(source, script);
  assert.deepEqual(script, {
    robot: { categoryId: "robotics", secondaryTags: ["sim-to-real", "manipulation"] },
  });
...
test("human repo override wins, while high-confidence exact P1 taxonomy match assigns deterministically without embedding", async () => {
  const repos = [
    repo(1, "robot", { classification: classification("robotics", 0.99, ["control"]) }),
    repo(2, "web", { classification: classification("uncategorized", 0.2) }),
  ];
  const taxonomy = await taxonomyFor(repos);
  const provider = embeddingProvider();
  const result = await assignScript(repos, taxonomy, provider, undefined, {
    overrides: { version: 1, repositories: { web: { categoryId: "web-apps", secondaryTags: ["frontend"] } } },
  });
  assert.equal(provider.state.calls, 0, "override + deterministic exact category should avoid embedding entirely");
  ...
  await assert.rejects(
    assignScript([repo(3, "bad")], taxonomy, provider, undefined, {
      overrides: { version: 1, repositories: { bad: { categoryId: "missing" } } },
    }),
    /unknown category/,
  );
});
...
test("semantic assignment uses absolute score plus top-vs-second margin and leaves weak/mixed repositories ambiguous", async () => {
  ...
  const taxonomy = await taxonomyFor(repos);
  const scriptProvider = embeddingProvider();
  const sourceProvider = embeddingProvider();
  const script = await assignScript(repos, taxonomy, scriptProvider);
  const source = await assignSource(repos, taxonomy, sourceProvider);
...
test("lexicographic category tie-break is deterministic when margin gate is explicitly zero", async () => {
  const repos = [repo(1, "tie")];
  const taxonomy = await taxonomyFor(repos);
  const result = await assignScript(repos, taxonomy, embeddingProvider(), undefined, { minScore: 0.5, minMargin: 0 });
  assert.equal(result.assignments.tie.categoryId, "robotics", "equal scores must use stable category id ordering");
...
test("embedding cache avoids repeated repository/category provider calls", async () => {
  ...
  assert.equal(first.diagnostics.repositoryEmbedded, 2);
  assert.equal(first.diagnostics.categoryEmbedded, 2);
  const calls = provider.state.calls;
  const second = await assignScript(repos, taxonomy, provider, cache);
  assert.equal(provider.state.calls, calls, "identical second assignment must be cache-only");
  assert.equal(second.diagnostics.repositoryCacheHits, 2);
  assert.equal(second.diagnostics.categoryCacheHits, 2);
  assert.equal(second.diagnostics.repositoryEmbedded, 0);
  assert.equal(second.diagnostics.categoryEmbedded, 0);
});
...
test("embedding provider failure preserves override/deterministic assignments and leaves unresolved repositories ambiguous", async () => {
  const repos = [
    repo(1, "robot", { classification: classification("robotics", 0.97) }),
    repo(2, "web"),
  ];
  const taxonomy = await taxonomyFor(repos);
  const result = await assignScript(repos, taxonomy, {
    id: "failing",
    model: "v1",
    async embed() { throw new Error("embedding unavailable"); },
  });
  assert.equal(result.assignments.robot.categoryId, "robotics");
  assert.equal(result.assignments.web, undefined);
  assert.deepEqual(result.ambiguous, ["web"]);
  assert.match(result.error ?? "", /embedding unavailable/);
});
...
test("Action shares repository embedding cache between semantic edges and taxonomy assignment", async () => {
  ...
  try {
    const result = await generateStaticMap(config(), {
      ...
      semanticOptions: { minSimilarity: -1, topK: 1 },
    });
    assert.equal(result.taxonomy.diagnostics.reason, "override");
    assert.equal(result.taxonomyAssignment.diagnostics.repositoryCacheHits, 2, "assignment must reuse repo vectors generated by semantic edges");
    ...
    const robotNode = result.graph.nodes.find((node) => node.type === "repository" && node.label === "robot");
    const webNode = result.graph.nodes.find((node) => node.type === "repository" && node.label === "web");
    assert.equal(robotNode?.taxonomyAssignment?.categoryId, "robotics");
    assert.equal(webNode?.taxonomyAssignment?.categoryId, "web-apps");
    assert.equal(robotNode?.groupId, "uncategorized", "P3B1 must not silently replace P1 hierarchy grouping");
    assert.equal(webNode?.groupId, "uncategorized");
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});

test("Action reads repository override from taxonomy-overrides.json, never rewrites it, and applies it with provider disabled", async () => {
  const dir = await mkdtemp(join(tmpdir(), "project-map-p3b1-override-"));
  const overrideText = JSON.stringify({
    version: 1,
    categories,
    repositories: {
      robot: { categoryId: "web-apps", secondaryTags: ["manual"] },
    },
  }, null, 2) + "\n";
  try {
    await writeFile(join(dir, "taxonomy-overrides.json"), overrideText);
    const result = await generateStaticMap({ ...config(), outputDir: "out" }, {
      cwd: dir,
      fetchRepos: async () => [repo(1, "robot")],
      taxonomyOverrides: overrideText,
    });
    const node = result.graph.nodes.find((item) => item.type === "repository" && item.label === "robot");
    assert.equal(node?.taxonomyAssignment?.method, "override");
    assert.equal(node?.taxonomyAssignment?.categoryId, "web-apps");
    assert.equal(result.taxonomyAssignment.diagnostics.providerId, "disabled");
    assert.equal(result.taxonomyAssignment.diagnostics.overridden, 1);
  } finally {
    await rm(dir, { recursive: true, force: true });
  }
});
...
test("static sanitizer preserves valid taxonomyAssignment, replaces spoofed label, and drops assignments to unknown taxonomy categories", async () => {
  const repos = [
    { ...repo(1, "robot"), html_url: "https://github.com/octocat/robot" },
    { ...repo(2, "web"), html_url: "https://github.com/octocat/web" },
  ];
  const taxonomy = await taxonomyFor(repos);
  const input = buildGraph("octocat", repos, true, true);
  input.taxonomy = taxonomy;
  input.taxonomyAssignmentVersion = 1;
  const robot = input.nodes.find((node) => node.type === "repository" && node.label === "robot");
  const web = input.nodes.find((node) => node.type === "repository" && node.label === "web");
  robot.taxonomyAssignment = {
    categoryId: "robotics",
    categoryLabel: "SPOOFED",
    secondaryTags: ["control"],
    confidence: 0.95,
    method: "semantic",
    score: 0.95,
    margin: 0.3,
    evidence: [{ categoryId: "robotics", source: "embedding", value: "cosine=0.95", weight: 0.95 }],
  };
  web.taxonomyAssignment = {
    categoryId: "missing",
    categoryLabel: "Missing",
    secondaryTags: [],
    confidence: 1,
    method: "override",
    evidence: [{ categoryId: "missing", source: "override", value: "bad", weight: 1 }],
  };

  const clean = sanitizeStaticGraph(input, "octocat");
  assert.ok(clean);
  const cleanRobot = clean.nodes.find((node) => node.type === "repository" && node.label === "robot");
  const cleanWeb = clean.nodes.find((node) => node.type === "repository" && node.label === "web");
  assert.equal(cleanRobot?.taxonomyAssignment?.categoryLabel, "Robotics");
  assert.equal(cleanRobot?.taxonomyAssignment?.categoryId, "robotics");
  assert.equal(cleanWeb?.taxonomyAssignment, undefined);
  assert.equal(clean.taxonomyAssignmentVersion, 1);
});
