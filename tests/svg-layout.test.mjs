...
function repositoryLabelBoxes(svg) {
  ...
  for (const match of svg.matchAll(pattern)) {
    const x = Number(match[1]); const y = Number(match[2]); const fontSize = Number(match[3]); const label = match[4];
...
for (const [style, render] of Object.entries(renderers)) {
  test(`${style} SVG keeps dense repository labels readable`, () => {
    const svg = render(denseGraph()); const boxes = repositoryLabelBoxes(svg);
    assert.ok(boxes.length >= 5, `${style} should retain several repository labels`);
    for (let first = 0; first < boxes.length; first += 1) for (let second = first + 1; second < boxes.length; second += 1) assert.equal(overlaps(boxes[first], boxes[second]), false, `${style} labels overlap: ${boxes[first].label} / ${boxes[second].label}`);
    assert.match(svg, />Original<\/text>/); assert.match(svg, />Fork<\/text>/); assert.match(svg, />Archived<\/text>/);
  });
}

test("treemap SVG keeps dense repositories bounded in non-overlapping category tiles", () => {
  const svg = renderTreemapSvg(denseGraph(), "dark", 740, 420);
  assert.match(svg, /aria-label="Treemap of example/);
  assert.match(svg, /Treemap · 28 projects/);
  assert.match(svg, />Original<\/text>/); assert.match(svg, />Fork<\/text>/); assert.match(svg, />Archived<\/text>/);
  assert.doesNotMatch(svg, /NaN|Infinity|width="-/);
  assert.equal([...svg.matchAll(/<title>long-project-name-/g)].length, 28);
});
