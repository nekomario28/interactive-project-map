...
test("Three.js lab is isolated from the existing viewer and consumes the shared Project Map view model", async () => {
  ...
  try {
    ...
    const [html, viewModelRuntime, runtime, css] = await Promise.all([
    ...
    assert.match(html, /<script src="\.\.\/project-map-view-model\.js"><\/script>\s*<script type="module" src="\.\.\/threejs-viewer\.js"><\/script>/);
    ...
    for (const path of [join(root, "project-map-view-model.js"), join(root, "threejs-viewer.js")]) {
