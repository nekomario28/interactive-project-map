...
const fakeEngine = `/* WebGLRenderer */\n${"x".repeat(100_100)}`;
...
test("build localization writes a pinned local Three.js asset and patches generated files", async () => {
  ...
  try {
    ...
    const [runtime, html, vendor] = await Promise.all([
    ...
    assert.equal(requestedUrl, THREE_SOURCE_URL);
...
test("localization fails closed on an invalid engine response", async () => {
  await assert.rejects(
    () => applyThreejsLocalEngine({ siteDir: "/does-not-matter", fetchImpl: async () => fakeResponse({ ok: false, status: 503 }) }),
    /HTTP 503/,
  );
});
