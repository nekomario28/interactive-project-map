...
test("Three.js search adapter consumes the renderer-neutral search projection and remains idempotent", async () => {
  ...
  try {
    ...
    const first = await applyThreejsSearchContext({ siteDir: root });
    ...
    const second = await applyThreejsSearchContext({ siteDir: root });
...
test("Three.js shared search postprocessor fails closed when Local Graph has not run", async () => {
  const root = await mkdtemp(join(tmpdir(), "ipm-three-search-order-"));
  try {
    await buildThreejsLab({ siteDir: root, sourceDir: join(process.cwd(), "scripts") });
    await assert.rejects(() => applyThreejsSearchContext({ siteDir: root }), /Local Graph adapter must run before shared search context/);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

test("Three.js shared search postprocessor passes Node syntax check", () => {
